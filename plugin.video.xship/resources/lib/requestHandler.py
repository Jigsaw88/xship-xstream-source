...
class cRequestHandler:
    def __init__(self, sUrl, caching=True, ignoreErrors=True, compression=True, jspost=False):
        ...
        #self._USER_AGENT = self.get_random_ua()
        self._aParameters = {}
        self._headerEntries = {}
    ...
    def request(self):
        ...
        if is_python2:
            ...
            # ssl_context.set_alpn_protocols(["http/1.1"]) # entfernt da Problem mit DDOS-GUARD
            ...
            if str(e.code) == '403' or str(e.code) == '503':
                ...
                if 'DDOS-GUARD' in str(data):
                    ...
                    try:
                        ...
                        if not self.ignoreErrors: xbmcgui.Dialog().ok(ADDON_NAME, 'DDOS-GUARD Fehler - Url: {0} {1}'.format(self._sUrl, str(e)))
                        if self.printErrors: print('DDOS-GUARD - HTTPError ' + str(e) + ' Url: ' + self._sUrl)
                        logger.warning('Failed DDOS-GUARD Url: ' + self._sUrl)
                        return ''
                elif 'cloudflare' in str(e.headers):
                    if not self.ignoreErrors: xbmcgui.Dialog().ok(ADDON_NAME, 'Cloudflare Fehler - Url: {0} {1}'.format(self._sUrl, str(e)))
                    if self.printErrors: print('Cloudflare - HTTPError ' + str(e) + ' Url: ' + self._sUrl)
                    logger.warning('Failed Cloudflare aktiv Url: ' + self._sUrl)
                    return ''
            else:
                if not self.ignoreErrors: xbmcgui.Dialog().ok(ADDON_NAME, 'Fehler beim Abrufen der Url: {0} {1}'.format(self._sUrl, str(e)))
                logger.warning('HTTPError ' + str(e) + ' Url: ' + self._sUrl)
                return ''
            # oResponse = e # ???

        except URLError as e:
            if not self.ignoreErrors: xbmcgui.Dialog().ok(ADDON_NAME, str(e.reason))
            logger.warning('URLError ' + str(e.reason) + ' Url: ' + self._sUrl)
            return ''
        except HTTPException as e:
            if not self.ignoreErrors: xbmcgui.Dialog().ok(ADDON_NAME, str(e))
            logger.warning('HTTPException ' + str(e) + ' Url: ' + self._sUrl)
            return ''
        except Exception as e:
            logger.warning('Exception '  + str(e) + ' Url: ' + self._sUrl)
            return ''
        ...
        if sys.version_info[0] == 3:
            try:
                ...
                sContent = str(sContent).encode('utf-8', 'replace').decode('utf-8')
        ...
        if 'lazingfast' in sContent:
            ...
            if bf:
                ...
                logger.warning('Failed BF Url: ' + self._sUrl)
        try:
...
class RedirectFilter(HTTPRedirectHandler):
    def redirect_request(self, req, fp, code, msg, hdrs, newurl):
        if 'notice.cuii' in newurl or 'alliance4creativity' in newurl: #'https://notice.cuii.info/' www.alliance4creativity.com
            return None
